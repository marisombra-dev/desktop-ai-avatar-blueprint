...
## 65. Global synthetic keyboard input can steal the human's keyboard

### What happened

During Google Cloud setup, an interrupted automation action left a modifier key logically held. The human temporarily could not use the keyboard normally.

### Lesson

Immediate human input ownership is a hard safety invariant. Release all common modifier keys before and after synthetic typing/shortcuts, including error/finally cleanup, and prefer background APIs or semantic accessibility actions over global keystroke injection.

---

## 66. OAuth granted in one host did not grant access to the desktop agent

### What happened

Gmail, Calendar, and Drive were already connected in another AI host, but the local desktop OpenClaw runtime had no native Google credentials.

### Lesson

OAuth authorization belongs to the runtime/application that received it. Create a separate local OAuth client/token store instead of assuming credentials can or should be copied across products.

---

## 67. A Google OAuth test app can reject its own owner until the test-user row is actually saved

### What happened

The OAuth app was in Testing mode and repeatedly returned access denied because the intended account was not actually present in the test-user table, even after an earlier UI attempt appeared to have added it.

### Lesson

Verify the postcondition in the Cloud console. Button clicks are not proof that configuration was committed.

---

## 68. Successful OAuth did not mean the Google APIs were enabled

### What happened

Gmail worked after consent, while Calendar and Drive returned HTTP 403 errors. Their APIs were still disabled for the Cloud project.

### Lesson

Treat OAuth scopes and per-project API enablement as separate gates. Test each service independently after authorization.

---

## 69. Valid Google results were rejected by an invalid MCP output contract

### What happened

The third-party Google Workspace MCP advertised output schemas for tools that returned ordinary MCP content instead of structured content. A current MCP SDK correctly rejected the mismatch before the policy proxy could forward the result.

### Lesson

A service call can succeed while the protocol adapter fails. Inspect the MCP contract boundary separately from OAuth/API behavior and repair the narrowest incompatible metadata layer instead of disabling validation globally. See `10j-native-google-workspace-mcp-bounded-oauth-validated-2026-09-16.md`.
