...
30. Optionally add bounded local computer control only after voice, interruption, and sensor routing are stable. Start with browser actions, keep manipulation separate from visual perception, verify every mutation against real OS/browser state, then add search/navigation and validate transitions out of shared-watch mode before expanding further. Preserve failed approaches so future builders do not repeat them. See `docs/10d-local-browser-control-validated-2026-09-12.md` and `docs/10e-browser-search-and-post-watch-stability-validated-2026-09-12.md`.
